export function niceline() {
  ...
  Line.prototype.render = function() {
    ...
    for (let i = this.index; i < LineArr.length; i++) {
      if (Math.abs(LineArr[i].x - this.x) < 80 && Math.abs(LineArr[i].y - this.y) < 80) {
        ctx.strokeStyle = this.color; //"rgb(192,192,192)";
        ctx.beginPath();
        ctx.globalAlpha =
          12 / Math.sqrt(Math.pow(LineArr[i].x - this.x, 2) + Math.pow(LineArr[i].y - this.y, 2));
    ...
    if (mouse.x > 2 && mouse.y > 2) {
      for (let i = this.index; i < LineArr.length; i++) {
        if (Math.abs(mouse.x - this.x) < 90 && Math.abs(mouse.y - this.y) < 90) {
          ctx.strokeStyle = this.color; //"rgb(192,192,192)";
          ctx.beginPath();
          ctx.globalAlpha =
            42 / Math.sqrt(Math.pow(LineArr[i].x - this.x, 2) + Math.pow(LineArr[i].y - this.y, 2));
          ctx.moveTo(this.x, this.y);
